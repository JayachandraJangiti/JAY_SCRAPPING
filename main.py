...
def reading_yaml_file(file_path):
    with open(file_path, 'r') as file:
        return yaml.safe_load(file)
        
path=input("\n\nEnter the path of Inputs Yaml file here:")
path=path.replace("\\","\\")

if os.path.exists(path):

    data = reading_yaml_file(path)

for input_data in data['inputs']:
    extract_table_content("https://www.data.bsee.gov/Well/APD/Default.aspx",input_data)